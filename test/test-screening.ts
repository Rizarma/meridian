...
async function main(): Promise<void> {
  ...
  const top: DiscoverPoolsResult = await discoverPools({ page_size: 10 });
  ...
  if (top.pools.length > 0) {
    ...
    console.log(`  Volume 24h: $${best.volume_window?.toLocaleString()}`);
  ...
  const trending: DiscoverPoolsResult = await discoverPools({ page_size: 5 });
  ...
  if (top.pools.length > 0) {
    ...
    try {
      const detail: RawPoolData = await getPoolDetail({ pool_address: poolAddr });
      ...
      console.log("Pool detail error:", errorMessage);
    }
  }
