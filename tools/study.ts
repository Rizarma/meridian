...
export async function studyTopLPers({
  pool_address,
  limit = 4,
}: StudyOptions): Promise<StudyResult> {
  if (!LPAGENT_KEYS.length) {
    return {
      pool: pool_address,
      message: "LPAGENT_API_KEY not set in .env — study_top_lpers is disabled.",
      patterns: {},
      lpers: [],
    };
  }
  ...
  const credible = all.filter((l) => l.total_lp >= 3 && l.win_rate >= 0.6 && l.total_inflow > 1000);
  ...
  const historicalSamples: LPerHistoricalSample[] = [];
  ...
  for (const lper of top) {
    ...
    try {
      ...
      const histData = (await histRes.json()) as { data?: HistoricalPosition[] };
