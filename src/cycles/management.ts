...
export async function runManagementCycle(
  ...
  // No outer timeout: a cosmetic Promise.race timeout was removed because it
  // rejected the caller without cancelling the inner mutex work, producing
  // misleading failure logs while the cycle continued running silently.
  return cycleState.getManagementMutex().runExclusive(async () => {
