...
export async function runManagementCycle(
  ...
  const MANAGEMENT_TIMEOUT_MS = 5 * 60 * 1000; // 5 minutes

  return Promise.race([
    cycleState.getManagementMutex().runExclusive(async () => {
...
After executing, write a brief one-line result per position.
    ...
    new Promise<never>((_, reject) =>
      setTimeout(() => reject(new Error("Management cycle timeout")), MANAGEMENT_TIMEOUT_MS)
    ),
  ]);
}
