...
async function importStrategies(
  ...
  const normalized = rows.map((row) => ({
    ...
    token_criteria_json: row.token_criteria_json ?? null,
    entry_criteria_json: row.entry_criteria_json ?? null,
    range_criteria_json: row.range_criteria_json ?? null,
    exit_criteria_json: row.exit_criteria_json ?? null,
  ...
  await insertRows(
    ...
    `ON CONFLICT (id) DO UPDATE SET
      ...
      range_criteria_json = excluded.range_criteria_json,
...
async function importPortfolioHistory(
  ...
  const normalized = rows.map((row) => ({
    ...
    total_positions_count: row.total_positions_count ?? 0,
  ...
  await insertRows(
    ...
    [
      ...
      "fetched_at",
      "first_seen_at",
      "fee_efficiency_annualized",
      "capital_rotation_ratio",
      "data_freshness_hours",
      "our_positions_count",
      "our_total_pnl_pct",
      "outperformance_delta",
      "is_active_pool",
      "lesson_generated",
    ],
    "ON CONFLICT DO NOTHING"
