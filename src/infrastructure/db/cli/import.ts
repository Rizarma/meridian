...
async function importPositionState(
  ...
  await insertRows(
    ...
    `ON CONFLICT (position) DO UPDATE SET
      ...
      active_bin = COALESCE(excluded.active_bin, position_state.active_bin),
      ...
      deployed_at = excluded.deployed_at,
      first_oor_at = COALESCE(excluded.first_oor_at, position_state.first_oor_at),
      last_claim_at = COALESCE(excluded.last_claim_at, position_state.last_claim_at),
      last_rebalance_at = COALESCE(excluded.last_rebalance_at, position_state.last_rebalance_at),
      rebalance_count = COALESCE(excluded.rebalance_count, position_state.rebalance_count),
      claim_count = COALESCE(excluded.claim_count, position_state.claim_count),
      notes = COALESCE(excluded.notes, position_state.notes),
      instructions = COALESCE(excluded.instructions, position_state.instructions),
      ...
      updated_at = GREATEST(position_state.updated_at, excluded.updated_at)`
  );
}
