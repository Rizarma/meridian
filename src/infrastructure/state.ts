...
async function load(): Promise<PositionState> {
  try {
    ...
    const recentEvents = await infra().db.query<Record<string, unknown>>(
      "SELECT * FROM position_state_events ORDER BY ts DESC LIMIT ?",
      MAX_RECENT_EVENTS
    );

    // Reverse to get chronological order
    const mappedEvents = recentEvents.map(rowToStateEvent);
    mappedEvents.reverse();
 ...
 */
async function pushEvent(event: Omit<StateEvent, "ts">): Promise<void> {
...
export async function clearPositions(): Promise<void> {
  await infra().db.transaction(async () => {
    await infra().db.run("DELETE FROM position_state");
    await infra().db.run("DELETE FROM position_state_events");
  });
...
export async function trackPosition({
  ...
  await infra().db.transaction(async () => {
    ...
    await pushEvent({ action: "deploy", position, pool_name: pool_name || pool });
  });
 ...
 */
export async function markOutOfRange(position_address: string): Promise<string | null> {
  const pos = await infra().db.get<{ out_of_range_since: string | null }>(
 ...
 */
export async function markInRange(position_address: string): Promise<null> {
  const pos = await infra().db.get<{ out_of_range_since: string | null }>(
    "SELECT out_of_range_since FROM position_state WHERE position = ?",
    position_address
  );
...
export async function minutesOutOfRange(
  ...
  const stored = await infra().db.get<{ out_of_range_since: string | null }>(
    "SELECT out_of_range_since FROM position_state WHERE position = ?",
    position_address
  );
 ...
 */
export async function recordClaim(position_address: string, fees_usd: number): Promise<void> {
  const pos = await getTrackedPosition(position_address);
  if (!pos) return;
 ...
 */
export async function recordClose(position_address: string, reason: string): Promise<void> {
  const pos = await getTrackedPosition(position_address);
  if (!pos) return;
 ...
 */
export async function recordRebalance(old_position: string, new_position: string): Promise<void> {
  const oldPos = await getTrackedPosition(old_position);
  const newPos = await getTrackedPosition(new_position);
 ...
 */
export async function queuePeakConfirmation(
  position_address: string,
  ...
  const pos = preloaded ?? (await getTrackedPosition(position_address));
 ...
 */
export async function queueTrailingDropConfirmation(
  position_address: string,
 ...
 */
export async function resolvePendingTrailingDrop(
  position_address: string,
  currentPnlPct: number | null,
  trailingDropPct: number | null,
  tolerancePct: number = TRAILING_DROP_CONFIRM_TOLERANCE_PCT
): Promise<TrailingConfirmation> {
  const pos = await getTrackedPosition(position_address);
 ...
 */
export async function getTrackedPositions(openOnly: boolean = false): Promise<TrackedPosition[]> {
...
export async function getStateSummary(): Promise<StateSummary> {
  ...
  const openPositions = await infra().db.query<Record<string, unknown>>(
...
export async function getLastBriefingDate(): Promise<string | null> {
  const row = await infra().db.get<{ value: string }>(
 ...
 */
export async function setLastBriefingDate(): Promise<void> {
 ...
 */
export async function syncOpenPositions(active_addresses: string[]): Promise<void> {
  const activeSet = new Set(active_addresses);
  const openPositions = await getTrackedPositions(true);
 ...
 */
export async function exportStateToJson(): Promise<void> {
  await exportToJson();
